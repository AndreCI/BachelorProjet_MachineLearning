clear;
for a=2:4
writerFile = fopen(['C:\Users\Public\matlab_output\results_mixture_TbF_route_month_ ' num2str(a) '.txt'],'w');
...
Xtotal_Cross = PML_getInput(dates_Cross,a,flightDuration_Cross,routes_Cross,month_Cross);
Xtotal_Test = PML_getInput(dates_Test,a,flightDuration_Test,routes_Test, month_Test);
%end parameters
...
for i=1:size_ridge_coef
    ...
    CVE = PML_cost_function(ridge_theta,Xtotal_Cross,prices_Cross,lambda_coef(i));
    TrE = PML_cost_function(ridge_theta,Xtotal,prices,lambda_coef(i));
    TeE = PML_cost_function(ridge_theta,Xtotal_Test,prices_Test,lambda_coef(i));
    if(CVE < old_CV_Error)
        idx = i;
        CVE_matrix(j+1) = CVE;
        old_CV_Error = CVE;
